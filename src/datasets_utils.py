...
def get_datasets(name, train_augment, test_augment, args):
    ...
    if (name == 'arroz'):
        #train_tvdataset = getattr(tvdatasets, name)(root='data/'+name, download=True)
        #test_tvdataset = getattr(tvdatasets, name)(root='data/'+name, download=False)
        
        # training dataset
        train_tvdataset = tvdatasets.ImageFolder(root='data/'+name)
        # teste dataset
        test_tvdataset = tvdatasets.ImageFolder(root='data/'+name)
        
    else:
        train_tvdataset = getattr(tvdatasets, name)(root='data/'+name, download=True)
        test_tvdataset = getattr(tvdatasets, name)(root='data/'+name, download=False)
    
    #train_tvdataset = getattr(tvdatasets, name)(root='data/'+name, split='letters', train=True, download=True
    #test_tvdataset = getattr(tvdatasets, name)(root='data/'+name, split='letters', train=False, download=False)
