...
if __name__ == '__main__':
    ...
    if (args.compute_weight_divergence == True):
        ...
        client_update2, client_num_examples2, client_num_iters2, client_loss2 = clients[1].train(model=client_model, optim=optim, device=args.device)
    ...
    for round in range(last_round + 1, args.rounds):
        ...
        for i, client_id in enumerate(client_ids):
            ...
            #    for chaveup in client_update.keys():
            #        print('chave: cadaum: ' , chaveup)
            #        np_arrup = client_update[chaveup].cpu().detach().numpy()
            #        np_arrup = np_arrup.flatten()
            #        np.savetxt('cliente'+'_'+str(args.iid)+'_'+str(args.model_args["norm"])+'_'+str(client_id)+chaveup+'.txt', np_arrup, delimiter=',')
        ...
        print('round EUUUU:' , round)
        #if (round == 20):
        if ((round % 1) == 0):
            import matplotlib.pyplot as plt
            medias = average_updates(updates, num_examples)
            for chave in medias.keys():
                ...
                #passa = 1
                #if (passa ==1):
                if (chave == 'feature_extractor.3.0.weight' or chave == 'feature_extractor.0.0.weight'):
                    
                    np_arr = medias[chave].cpu().detach().numpy()
                    
                    
                    np_arr = np_arr.flatten()
                    np.savetxt('media'+str(round)+'_'+str(args.iid)+'_'+str(args.model_args["norm"])+'_'+str(client_id)+chave+'.txt', np_arr, delimiter=',')
                    
                    
                    #n, bins, patches = plt.hist(np_arr, 100, density=True, facecolor='g', alpha=0.75)
                    #plt.savefig('media'+str(client_id)+chave+'.png')
                    #plt.clf()
                    
                    
                    #fig = sm.qqplot(np_arr, line='45')
                    #plt.savefig('QQ_plot_media_'+str(client_id)+chave+'.png')
                    #fig.clf()
                    #plt.clf()
    ...
    '''
    if (args.compute_weight_divergence == True and round == round):
        ...
        div1 = np.zeros_like(np_arr_fed)
        
        for x in range(0, np_arr_fed.size):
            div1[x] = abs(np_arr_fed[x])
            
        desvio_padrao1 = np.std(div1,ddof=1) # 
        media1 =  np.mean(div1) # 
        
        print('Media Final div feature_extractor.0.0.weight: ',media1)
        print('DP Final div feature_extractor.0.0.weight: ',desvio_padrao1)
    '''
