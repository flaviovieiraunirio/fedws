...
if __name__ == '__main__':
    ...
    #np_arr4  = np.concatenate((np_arr0, np_arr1), axis=0)
    #np_arr5  = np.concatenate((np_arr2, np_arr3), axis=0)
    
    div1 = np.zeros_like(np_arr0)
    div2 = np.zeros_like(np_arr1)
    
    for x in range(0, np_arr0.size):
        if (np_arr0[x] == 0):
            div1[x] = abs(np_arr2[x])
        else:
            div1[x] = abs(np_arr2[x] - np_arr0[x]) / np_arr0[x]
