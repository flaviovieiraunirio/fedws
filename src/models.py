...
import models2.imagenet as models2 # novo
import vgg11 as vggmod # novo
...
#From Micro-Batch Training with Batch-Channel Normalization and Weight Standardization#
...
# Layer normalization from source code of "Rethinking Normalization Methods in Federated Learning" (Du et al, 2022)
class lenet5_d(nn.Module):
    def __init__(self, num_classes, num_channels, model_args):
        ...
        if norm == 'batch':
            ...
            conv1 = nn.Conv2d(num_channels, 64, 5, padding='same')
            conv2 = nn.Conv2d(64, 64, 5, padding='same')
        elif norm == 'group':
            ...
            norm1 = nn.GroupNorm(int(64/16), 64)
            norm2 = nn.GroupNorm(int(64/16), 64)
            conv1 = Conv2d_WS(num_channels, 64, 5, padding='same')
            conv2 = Conv2d_WS(64, 64, 5, padding='same')
        elif norm == 'layer':
            # Group Normalization paper suggests 16 channels per group is best
            norm1 = nn.GroupNorm(1, 64)
            norm2 = nn.GroupNorm(1, 64)
            conv1 = nn.Conv2d(num_channels, 64, 5, padding='same')
            conv2 = nn.Conv2d(64, 64, 5, padding='same')
        elif norm == None:
...
class resnet50(nn.Module):
    def __init__(self, num_classes, num_channels, model_args):
        ...
        if pretrained:
            ...
            self.model = tvmodels.resnet.ResNet(alpha=width, num_classes=num_classes, dropout=dropout)
...
class resnet18(nn.Module):
    def __init__(self, num_classes, num_channels, model_args):
        ...
        if pretrained:
            self.model = tvmodels.resnet18(pretrained=True)
            if freeze:
                for param in self.model.parameters():
                    param.requires_grad = False
            ...
            self.model = tvmodels.resnet18(pretrained=False)
            #self.model = tvmodels.resnet.ResNet(alpha=width, num_classes=num_classes, dropout=dropout)
...
class resnet18ws(nn.Module):
    def __init__(self, num_classes, num_channels, model_args):
        super(resnet18ws, self).__init__()
        width = model_args['width'] if 'width' in model_args else 1
        dropout = model_args['dropout'] if 'dropout' in model_args else 0.2
        pretrained = model_args['pretrained'] if 'pretrained' in model_args else False
        freeze = model_args['freeze'] if 'freeze' in model_args else False
        ...
        if pretrained:
            ...
            #self.model = model.
            #self.model = models.imagenet.l_resnet18(pretrained=True)
            self.model = models2.l_resnet18(pretrained=True)
            #self.model = tvmodels.resnet18(pretrained=True)
            self.model.load_state_dict(torch.load('models/resnet18/model_best.pth'), strict=False)
            if freeze:
                for param in self.model.parameters():
                    param.requires_grad = False
            ...
            model = models.imagenet.l_resnet18(pretrained=True)
            '''
            ...
            self.model = models2.l_resnet18(pretrained=False)
            #self.model = tvmodels.resnet.ResNet(alpha=width, num_classes=num_classes, dropout=dropout)
...
class resnet50ws(nn.Module):
    def __init__(self, num_classes, num_channels, model_args):
        super(resnet50ws, self).__init__()
        width = model_args['width'] if 'width' in model_args else 1
        dropout = model_args['dropout'] if 'dropout' in model_args else 0.2
        pretrained = model_args['pretrained'] if 'pretrained' in model_args else False
        freeze = model_args['freeze'] if 'freeze' in model_args else False
        ...
        if pretrained:
            ...
            self.model = models2.l_resnet50(pretrained=False)
            #self.model = tvmodels.resnet.ResNet(alpha=width, num_classes=num_classes, dropout=dropout)
