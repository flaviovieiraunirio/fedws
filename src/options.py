...
def args_parser():
    ...
    args_other.add_argument('--client_failures', type=int, default=0,
                        help="percentagem de falhas dos clientes")
    args_other.add_argument('--compute_weight_divergence', action='store_true', default=False,
                        help="disable checkpoints")
...
def args_str_to_dict(args_str):
    ...
    if args_str is not None:
        for arg in args_str.replace(' ', '').split(','):
            ...
            if (keyvalue[1] == 'batch' or keyvalue[1] == 'group' or keyvalue[1] == 'groupws' or keyvalue[1] == 'layer'):
                args_dict[keyvalue[0]] = keyvalue[1]
            else:
                args_dict[keyvalue[0]] = literal_eval(keyvalue[1])
            #print('literal_eval(keyvalue[1]):', literal_eval(keyvalue[1]))
    return args_dict
